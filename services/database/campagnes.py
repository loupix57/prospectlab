...
class CampagneManager(DatabaseBase):
    ...
    def update_campagne(
        ...
        # Optionnel: si la colonne date_modification existe, on la met à jour
        try:
            cursor.execute("PRAGMA table_info(campagnes_email)")
            cols = {row[1] for row in cursor.fetchall()}
            if 'date_modification' in cols:
    ...
    def get_emails_campagne(self, campagne_id):
        ...
        # On récupère aussi le nom d'entreprise quand on peut
        cursor.execute(
            '''
            SELECT
                e.*,
